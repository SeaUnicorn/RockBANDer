...
def play_TABs(g_Inf):
    ...
    for item in range(0, len(CMDs)):
        if CMDs[item].note == 50:
            G_CMD = G_CMD + ('N'+ str(g_Inf.JIT_N) + ' ') + (str('G04 ' + str(CMDs[item].duration*float(g_Inf.duration))+ ' HERE \n \n'))
        else:
            G_CMD = G_CMD + (play(CMDs[item].string, g_Inf.angle_ZX, g_Inf.angle_ZY, g_Inf.JIT_N, g_Inf.mode, g_Inf.first_point, 1, CMDs[item].string, CMDs[item].note))
            G_CMD = G_CMD + ('N'+ str(g_Inf.JIT_N) + ' ') + (str('G04 ' + str(CMDs[item].duration*float(g_Inf.duration))+ '\n \n'))
